...
Usage() {
    echo "Usage: testall.sh [options] [.drrt files]"
...
Check() {
    ...
    Run "$CC" "-o" "${basename}.exe" "${basename}.s" "htmllibrary.o" "listlibrary.o" && 
...
CheckFail() {
    ...
    RunFail "$DRRTY" "<" $1 "2>" "${basename}.err" ">>" $globallog &&
...
then
    ...
    files="tests/test-*.drrt" #tests/fail-*.drrt"
fi
